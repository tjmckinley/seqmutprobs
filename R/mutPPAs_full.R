...
{	
	...
	if(criteria[1]=="both")
	{
		hyp_lessstring<-.C("calc_lessstring_fn",as.integer(length(mutlPDM$nseq)),as.integer(nrow(mutlPDM$models_num)),as.integer(mutlPDM$models_num),hyp=as.integer(numeric(nrow(mutlPDM$models_num))), PACKAGE = "seqmutprobs")$hyp
		hyp_string<-.C("calc_string_fn",as.integer(length(mutlPDM$nseq)),as.integer(nrow(mutlPDM$models_num)),as.integer(mutlPDM$models_num),hyp=as.integer(numeric(nrow(mutlPDM$models_num))), PACKAGE = "seqmutprobs")$hyp
	}
	else
	{
