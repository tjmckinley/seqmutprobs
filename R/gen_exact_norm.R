# function to generate EXACT normalising constants for use in efficient EXACT
# search routine
gen_exact_norm <- function(seqdata, pstar, lpriorPA, mc.cores = 1, ...) {
    ...
    lnorms <- mclapply(seqdata1, function(seqs, nsamp, pstar, ntotcol, lpriors) {
        ...
        lnorms <- apply(lpriors, 1, function(x, nsamp, ntotcol, lPDM_int_mat) lnorm <- .Call("genmodels_exact", 
            nsamp, ntotcol, x, lPDM_int_mat, PACKAGE = "seqmutprobs"), nsamp = nsamp, 
            ntotcol = ntotcol, lPDM_int_mat = lPDM_int_mat)
        lnorms
