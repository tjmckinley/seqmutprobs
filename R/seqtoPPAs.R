...
#' 
#' Takes a list of samples, where each element of the list contains a set of
#' sequences, and computes a set of corresponding posterior probabilities of
#' association
#' 
#' Function takes a list of paths-to-files where each file corresponds to a set
...
#' model is less than \code{c}. If \code{supp_output=TRUE}, then none of the
#' individual model outputs for each unique site are returned, only the overall
#' PPAs relating to whether sites are sites-of-interest or not.
#' 
#' The function can also read in pile-up tables produced using \code{samtools}.
#' In Unix-type systems these can be generated using the command:
#' \code{samtools mpileup -f REFERENCE BAMFILE > PILEUP.txt}, where REFERENCE
#' is replaced by the path to the reference file, BAMFILE is replaced with the
#' path to the corresponding BAM file, and PILEUP is replaced by the name of
#' the output file. The reference file must be indexed in advanced (e.g. if the
#' refernce file is in fasta format, then \code{samtools faidx ref.fasta}
#' should do the trick. If BAM files are used as the input to \code{seqtoPPAs},
#' the function does these steps automatically.
#' 
#' If the number of samples being evaluated is greater than or equal to the
...
#' argument defines a minimum number of reads per site that must be present in
#' the intial sequence in order for that site to be considered for analysis.
#' Due to the large size of many BAM files, the \code{genes} argument allows
#' subsets of the data to be analysed via gene segment.
#' 
#' The screening criteria are defined in McKinley et al. (2010, 2012).
...
#' subsettable).
#' 
#' @aliases seqtoPPAs seqtoPPAs.default
#' @param filenames a list of paths to files containing alignments, in correct
...
#' @param pstar a scalar quantity defining the per-nucleotide mutation
#' probability under the random mutation hypothesis. If \code{NULL} then it is
#' calculated directly from the samples in \code{seqdata}.
#' @param sites an integer vector specifying which subset of nucleotide sites
#' are to be examined. If \code{NA} then all sites (other than insertions
#' and/or sites with invalid coverage) are evaluated.
#' @param genes a vector of characters to be used with NGS data to specify the
#' genes we wish to extract. Can be missing, in which case we compare all
#' segments present in the initial sample.
#' @param cov_thresh a scalar denoting the minimum coverage required for a site
#' to be considered for analysis (only used for NGS data due to hetergeneous
#' coverage).
#' @param nswitch_to_supp_output a scalar defining the default number of
#' samples above which to automatically suppress the model outputs.
#' @param mc.cores a scalar defining the number of cores over which to
#' parallelise the code. If set to \code{NA}, then this attempts to
#' automatically ascertain how many cores are available, else it uses a single
#' core.
#' @param \dots not used.
#' @return A list where each element contains an object of class
#' \code{mutPPAs.list} corresponding to each gene, these are basically lists
#' including elements:
#' \itemize{
#' \item{nseq:}{ if the input alignments are from capillary sequencing, then
#' this is a vector the same length as \code{seqdata} containing the number of
#' sequences in each sample. If the alignments are from NGS data, then this is
#' a list where each element of the list contains the maximum and minimum
#' coverage counts across the gene segment of interest.}
#' \item{nnuc:}{ a scalar containing the total number of nucleotide sites in
#' each gene segment.}
#' \item{pstar:}{ a scalar containing the value of p*.}
#' \item{samp_names:}{ a character vector the same length as \code{seqdata}
#' containing the names of each sample.}
...
#' \item{cons:}{ a character vector of length \code{nnuc} containing the
#' consensus sequence.}
...
#' nested list with (number of unique sites x length of \code{priorPA} x number
#' of criteria) elements. The forms for each of these final elements is
#' described below:}
#' \itemize{
#' \item{models_num:}{ the top-level elements contain matrix
#' representations of all possible models for all unique sites. For each model
#' matrix, if \code{nsamp} is the number of samples, then each row corresponds
#' to a model, and the first \code{nsamp} columns correspond to the choice of
#' model for each sample, and the second \code{nsamp} columns correspond to a
#' set of dependency indicators corresponding to whether samples are assumed to
#' be drawn from exactly the same underlying distribution(s) or not. These are
#' converted to a prettier form when printed.}
#' \item{hyps:}{ the top-level elements contain binary vectors
#' corresponding to which models in the corresponding element of
#' \code{models_num} are consistent with the null (0) and alternative (1)
#' hypotheses.}
#' \item{nprior:}{ a numeric vector of length 3, containing the
#' total number of possible models, and the number of those corresponding to
#' the null and alternative hypotheses.}
#' \item{PPAs:}{ the top-level elements contain numeric vectors
#' corresponding to the PPAs for each model in the corresponding element of
#' \code{models_num}.}
...
#' suppressed.}
#' \item{hyp_PPAs:}{ a nested list with (length of \code{priorPA} x number
#' of criteria x number of unique sites) elements, where each top-level element
#' is a scalar corresponding to the PPA that the given site is a
#' site-of-interest given the relevant screening criteria (denoted PPA_SI in
#' references).}
#' }
#' @author TJ McKinley
#' @seealso \code{\link{extract_site_info}}, \code{\link{summary.mutPPAs}},
