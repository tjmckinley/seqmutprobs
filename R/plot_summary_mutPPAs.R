...
plot.summary.mutPPAs <- function(x, prior = NULL, entropy = c("max", "mean"), type = c("all", 
    "ent_PPA", "shannon_ent", "prior_ent", "temporal_ent"), nrow_legend = 7, ...) {
    if (class(x) != "summary.mutPPAs") 
        stop("'x' is not a 'summary.mutPPAs' object")
    ...
    if (entropy[1] != "max" & entropy[1] != "mean") 
        stop("Wrong input for 'entropy' argument")
    if (!is.character(type)) 
        stop("'type' argument is not a character vector")
    matchtype <- match(type, c("all", "ent_PPA", "shannon_ent", "prior_ent", "temporal_ent"))
    if (length(matchtype[is.na(matchtype)]) > 0) 
        stop("Wrong input for 'type' argument")
    if (length(which(type == "all")) > 0) 
        type <- c("ent_PPA", "shannon_ent", "prior_ent", "temporal_ent")
    ...
    if (entropy[1] == "max") 
        entcol <- ncol(x$entropy) else entcol <- ncol(x$entropy) - 1
    ...
    for (j in 2:ncol(sites1)) {
        ...
        if (!is.matrix(sites2)) 
            sites2 <- matrix(sites2, nrow = 1)
        if (j == 2) 
            entropy.dat <- data.frame(sites = sites2[, 1], x = entropy1, y = sites2[, 
                j], names = colnames(sites1)[j]) else entropy.dat <- rbind(entropy.dat, data.frame(sites = sites2[, 1], x = entropy1, 
            y = sites2[, j], names = colnames(sites1)[j]))
    }
    ...
    if (length(which(type == "ent_PPA")) > 0) {
        plots.list[[plots.ind]] <- qplot(x, y, data = entropy.dat, geom = "point", 
            colour = sites, xlab = "K-L entropy (relative to first sample)", ylab = paste("PPA (prior PA=", 
                prior1, ")", sep = "")) + facet_grid(names ~ .) + guides(col = guide_legend(ncol = ceiling(length(ent.levels)/nrow_legend), 
            title = "Sites"))
        plots.ind <- plots.ind + 1
    }
    
    # plot entropy vs Shannon entropy
    entropy.dat <- data.frame(sites = ent.sites, x = x$shannon[, entcol + 1], y = x$entropy[, 
        entcol])
    entropy.dat$sites <- factor(entropy.dat$sites, levels = ent.levels)
    
    if (length(which(type == "shannon_ent")) > 0) {
        plots.list[[plots.ind]] <- qplot(x, y, data = entropy.dat, geom = "point", 
            colour = sites, ylab = "K-L entropy (relative to first sample)", xlab = "Shannon entropy") + 
            guides(col = guide_legend(ncol = ceiling(length(ent.levels)/nrow_legend), 
                title = "Sites"))
        plots.ind <- plots.ind + 1
    }
    
    # plot entropy vs Shannon entropy
    entropy.dat <- data.frame(sites = ent.sites, x = x$klprior[, entcol + 1], y = x$entropy[, 
        entcol])
    entropy.dat$sites <- factor(entropy.dat$sites, levels = ent.levels)
    
    if (length(which(type == "prior_ent")) > 0) {
        plots.list[[plots.ind]] <- qplot(x, y, data = entropy.dat, geom = "point", 
            colour = sites, ylab = "K-L entropy (relative to first sample)", xlab = "1 - K-L entropy (relative to prior)") + 
            guides(col = guide_legend(ncol = ceiling(length(ent.levels)/nrow_legend), 
                title = "Sites"))
        plots.ind <- plots.ind + 1
    }
    ...
    if (length(which(type == "temporal_ent")) > 0) {
        ...
        entropy1 <- data.frame(entropy = unlist(entropy1), comparison = rep(colnames(entropy1), 
            each = nrow(entropy1)))
        entropy1 <- cbind(sites = rep(sites1[, 1], times = nrow(entropy1)/nrow(sites1)), 
            entropy1)
        entropy1$sites <- factor(as.character(entropy1$sites), levels = ent.levels)
        if (nrow(entropy1) > 2) {
            plots.list[[plots.ind]] <- ggplot(entropy1, aes(comparison, entropy, 
                group = sites, colour = sites)) + geom_line() + guides(col = guide_legend(ncol = ceiling(length(ent.levels)/nrow_legend), 
                title = "Sites")) + ylab("K-L entropy (relative to first sample)") + 
                xlab("Samples (in order)")
            plots.ind <- plots.ind + 1
        }
    }
