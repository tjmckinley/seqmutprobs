...
mutPPAs_top <- function(seqdata, c = 20, priorPA = c(0.001, 0.05, 0.01), criteria = c("both", 
    "stringent", "less"), ppas = TRUE, justsetup = FALSE, mc.cores = mc.cores, ...) {
    ...
    if (justsetup == FALSE) {
        ...
        PPA_mat <- mclapply(seqdata1, function(seqs, nsamp, pstar, ntotcol, logc, 
            lpriors, structure, criteria, priorPA, ppas) {
            ...
            lPPAs <- apply(lpriors, 1, function(x, seqs, nsamp, pstar, ntotcol, logc, 
                structure, criteria, lPDM_int_mat, ppas) {
                ...
                if (criteria == "both" | criteria == "less") {
                  ...
                  # criteria
                  if (ppas == TRUE) 
                    lPPA_mat_ls <- .Call("calc_PPAs_approx_fn", nsamp, ntotcol, logc, 
                      x[1], x[2], lPDM_int_mat1, 0, structure, length(structure)/nsamp, 
                      1, uni, uni_ind, PACKAGE = "seqmutprobs") else lPPA_mat_ls <- .Call("calc_PPAs_approx_fn", nsamp, ntotcol, 
                    logc, x[1], x[2], lPDM_int_mat, 0, structure, length(structure)/nsamp, 
                    0, uni, uni_ind, PACKAGE = "seqmutprobs")
                  ...
                  if (ppas == TRUE) 
                    lPPA_mat_ls <- list(models_num = models_num, hyps = hyp, lPPA = lPPA_mat_ls) else {
                ...
                if (criteria == "both" | criteria == "stringent") {
                  ...
                  # criteria
                  if (ppas == TRUE) 
                    lPPA_mat_s <- .Call("calc_PPAs_approx_fn", nsamp, ntotcol, logc, 
                      x[3], x[4], lPDM_int_mat1, 1, structure, length(structure)/nsamp, 
                      1, uni, uni_ind, PACKAGE = "seqmutprobs") else lPPA_mat_s <- .Call("calc_PPAs_approx_fn", nsamp, ntotcol, 
                    logc, x[3], x[4], lPDM_int_mat, 1, structure, length(structure)/nsamp, 
                    0, uni, uni_ind, PACKAGE = "seqmutprobs")
                  ...
                  if (ppas == TRUE) 
                    lPPA_mat_s <- list(models_num = models_num, hyps = hyp, lPPA = lPPA_mat_s) else {
                    # calculate final lPPAs and normalising constant
            ...
            }, seqs = seqs, nsamp = nsamp, pstar = pstar, ntotcol = ntotcol, logc = logc, 
                structure = structure, criteria = criteria, lPDM_int_mat = lPDM_int_mat, 
                ppas = ppas)
            names(lPPAs) <- paste("priorPA_", priorPA, sep = "")
            lPPAs
        }, nsamp = nsamp, pstar = pstar, ntotcol = ntotcol, logc = logc, lpriors = lpriors, 
            structure = structure, criteria = criteria[1], priorPA = priorPA, ppas = ppas, 
            mc.cores = mc.cores)
    } else PPA_mat <- NA
    
    # output object
    output <- list(nseq = nseq, nnuc = nnuc, pstar = pstar, samp_names = samp_names, 
        basedist = seqdata, nucind = seqind, cons = full_cons, rem_sites = rem_sites, 
        test_sites = sites, priorPA = priorPA, PPAs = PPA_mat, lpriors = lpriors, 
        seqdata = seqdata, nprior = nprior, genes = genes)
    output
} 
